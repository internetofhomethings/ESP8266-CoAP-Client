/********************************************************************
 * web_server/coap client - example code providing a framework that supports
...
uint8_t packetbuf_LED_BLINK[]  = {0x40, 0x03, 0x00, 0x00, 0xBB, 0x6C, 0x69, 0x67, 0x68, 0x74, 0x5F, 0x62, 0x6C, 0x69, 0x6E, 0x6B, 0xFF, 0x35};
uint8_t packetbuf_GETSENSORS[] = {0x40, 0x03, 0x00, 0x00, 0xB7, 0x72, 0x65, 0x71, 0x75, 0x65, 0x73, 0x74, 0xFF, 0x2F, 0x3F, 0x72, 0x65, 0x71, 0x75, 0x65, 0x73, 0x74, 0x3D, 0x47, 0x65, 0x74, 0x53, 0x65, 0x6E, 0x73, 0x6F, 0x72, 0x73};
...
/********************************************************
 * Set local CoAP Server IP
 * Function: SetCoapSvrIP(void)
 * 
...
void SetCoapSvrIP(void) {
    ...
    Serial.print(ipcoapsvr[0]);    
    Serial.print(".");    
    Serial.print(ipcoapsvr[1]);    
    Serial.print(".");    
    Serial.print(ipcoapsvr[2]);    
    Serial.print(".");    
    Serial.println(ipcoapsvr[3]);    
}

/********************************************************
 * Domain name lookup callback
 * Function: dns_found(void)
 * 
 * Parameter    Description
 * ---------    ---------------------------------------
 * return       no return value
...
void dns_found(const char *name, ip_addr_t *ipaddr, void *arg)
{
    ...
    if(String(name) == String(domaincoapsvr)) {
        //Set coap server ip
        ipcoapsvr[0]  = *((uint8 *)&ipaddr->addr);
        ipcoapsvr[1]  = *((uint8 *)&ipaddr->addr+1);
        ipcoapsvr[2]  = *((uint8 *)&ipaddr->addr+2);
        ipcoapsvr[3]  = *((uint8 *)&ipaddr->addr+3);
        
        Serial.print("CoAP Server Domain: ");    
        Serial.println(name);    

    }
    else if(name == "domainTBD") {
        //Set TBD ip
        /*
        ipTBD[0]  = *((uint8 *)&ipaddr->addr);
        ipTBD[1]  = *((uint8 *)&ipaddr->addr+1);
        ipTBD[2]  = *((uint8 *)&ipaddr->addr+2);
        ipTBD[3]  = *((uint8 *)&ipaddr->addr+3);
        */
    }
    else {
 ...
 * Function: GetHostByName(void)
 * 
 * Parameter    Description
 * ---------    ---------------------------------------
 * return       no return value
 ********************************************************/
void  GetHostByName(char * domainname) {         
 ...
 void ProcessCOAPMsg(void) {
    ...
    { 
        ...
        if (0 != (rc = coap_parse(&pkt, packetbuf, sz)))
        {
            ...
            //Set Reply Payload
            os_memcpy((char *)coap_reply_c,(char *) &packetbuf[8], sz-8);
            coap_reply_c[sz-8] = '\0';
            Serial.println(coap_reply_c);
        }
    }
    busy=0;
    yield();
...
void udp_send(const uint8_t *buf, int buflen)
{
    udp.beginPacket(ipcoapsvr, 5683);
    while(buflen--)
        udp.write(*buf++);
    udp.endPacket();
}
 ...
 ******************************************************************************************/
 ...
 void incrMessageID(uint8_t *packetbuf) {
    //Increment Mesaage ID   
    packetbuf[3]  =  packetbuf[3] + 1;
    packetbuf[2]  =  (packetbuf[3] == 0) ? packetbuf[2] + 1 : packetbuf[2];
 }
            ...
            if(str != NULL) {
                ...
                str++;
                  ...
                  if(str != NULL) {
                      ...
                      os_memcpy(purl_param->pParVal[ipar++], pbuffer, length);
                      //if(ipar!=0)
                      //    purl_param->pParVal[ipar][length] = '\0';
                      //ipar++;
                      str = NULL;
                  }
                  else {
...
void setup() {
    ...
    #if EXTERNAL_COAP_SVR==1
    GetHostByName((char *)domaincoapsvr);// Get CoAP Server IP
    #else
    SetCoapSvrIP();                      // Set CoAP Server Local IP
    #endif
